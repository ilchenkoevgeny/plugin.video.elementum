...
def _send_provider_callback(payload, results, done=True):
    progressive = payload.get("progressive", False)
    response = tuple(results or ())
    ...
    if callback_user or callback_password:
        base64string = base64.b64encode(
            '{}:{}'.format(callback_user, callback_password).encode()
        )
        req.add_header(
            "Authorization",
...
def register(search, search_movie, search_episode, search_season=None):
    ...
    try:
        results = ()
        try:
            search_object = payload["search_object"]
            if progressive:
                search_object["progressive"] = True
                search_object["progressive_timeout"] = payload.get(
                    "progressive_timeout",
                    130,
                )

            objects = method(search_object)
            if objects is not None and progressive:
                if isinstance(objects, (list, tuple)):
                    _send_provider_callback(
                        payload,
                        objects,
                        done=False,
                    )
        ...
        try:
            _send_provider_callback(payload, results, done=True)
        except Exception as e:
            import traceback
