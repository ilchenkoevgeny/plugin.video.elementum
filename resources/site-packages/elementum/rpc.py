...
class ElementumRPCServer(BaseHandler):
    ...
    def Dialog_Select_Large_Progressive_Create(self, title, subject, items):
        title_encoded = "%s %s" % (getLocalizedLabel(title), toUtf8(subject))
        ...
        self._objects[window_id] = window
        window.show()
        return window_id

    def Dialog_Select_Large_Progressive_Update(self, hwnd, items):
        window = self._objects.get(hwnd)
        if window:
            window.updateItems(items)

    def Dialog_Select_Large_Progressive_Result(self, hwnd):
        window = self._objects.get(hwnd)
        if not window:
            return {'done': True, 'choice': -1}
        return window.result()

    def Dialog_Select_Large_Progressive_Close(self, hwnd):
        window = self._objects.pop(hwnd, None)
        if window:
            window.close()
            del window
